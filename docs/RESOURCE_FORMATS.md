...
PalForge 0.6 只读取用户本地选择的游戏目录。任何格式错误都会停在当前 chunk、场景或脚本；工程脚本与调试快照保存在独立项目/内存中，不会修改源文件。
...
- 0.6 可以编辑工程脚本、生成追加式编译预览并在场景沙盒中运行，但不写回 MKF/M.MSG，也不会启动 SDLPAL；场景名称和未知 opcode 不从二进制中臆造。
