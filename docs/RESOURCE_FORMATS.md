...
PalForge 0.7 只读取用户本地选择的游戏目录。任何格式错误都会停在当前 chunk、场景或脚本；工程脚本、调试快照和 SDLPAL 临时运行文件保存在独立项目/内存中，不会修改源文件。
...
- 0.7 可以编辑工程脚本、生成追加式编译预览、在脚本沙盒中运行，并从指定场景启动真实 SDLPAL-WASM；任何阶段都不写回 MKF/M.MSG，场景名称和未知 opcode 不从二进制中臆造。
