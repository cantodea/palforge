...
> 当前版本是 **0.3 Scene Lens**。它会读取你选择的真实 PAL 目录并组合原版场景，但不会覆写原版文件。请保留合法取得的游戏数据，并始终使用副本。
...
1. 点击 **打开游戏目录**，选择同时包含 `SSS.MKF`、`MAP.MKF`、`GOP.MKF`、`MGO.MKF` 与 `PAT.MKF` 的完整目录。
...
当前只读资源与真实场景链路已经打通；真实地图双向编辑、完整 opcode 语义、资源写回和 SDLPAL 进程桥接仍在后续阶段。格式细节见 [资源格式说明](docs/RESOURCE_FORMATS.md)，模块拆分见 [架构说明](docs/ARCHITECTURE.md)。

## 路线图

- **0.1 · Forge Desk（完成）**：编辑器外壳、项目模型、MKF 索引、地图/脚本/资源/模块/测试工作流。
- **0.2 · Resource Lens（完成）**：YJ_1/YJ_2、sprite/RLE、PAT 调色板、FBP 预览与 chunk 导出。
- **0.3 · Scene Lens（当前）**：真实 PAL 地图、GOP 图块、事件对象、碰撞/高度和脚本引用的只读组合预览。
- **0.4 · Scene Builder**：以 PalForge 差异工程实现地图和事件的双向编辑，并安全导出到副本。
- **0.5 · Script Graph**：覆盖 `PAL_InterpretInstruction` 的 opcode 注册表、跳转校验和剧情变量检查。
