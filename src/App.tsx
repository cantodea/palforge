  ...
  return (
    <div className="map-toolbar">
      <div className="segmented tools">
        ...
        <button className={tool === 'paint' ? 'active' : ''} onClick={() => onTool('paint')} title="绘制地形">
      ...
      <div className="zoom-controls">
        <button className="icon-button" onClick={() => onZoom(Math.max(0.65, zoom - 0.1))}><ZoomOut size={15} /></button>
...
export default function App() {
  ...
  return (
    <div className="app-shell">
      ...
      <TopBar projectMounted={projectMounted} onOpen={() => gameInputRef.current?.click()} onSave={saveProject} onExport={exportProject} />
      ...
      <ProjectExplorer view={view} onView={setView} selectedScriptId={selectedScriptId} onScript={(script) => { setSelectedScriptId(script.id); setView('script') }} resources={resources} />
      <main className={`main-area ${testOpen ? 'test-open' : ''}`}>
        {view === 'map' && (
          <div className="map-workspace">
            <MapToolbar tool={tool} onTool={setTool} brush={brush} onBrush={setBrush} zoom={zoom} onZoom={setZoom} showGrid={showGrid} onGrid={() => setShowGrid((value) => !value)} />
            ...
            <div className="map-stats"><span><Layers3 size={13} /> 14 × 10</span><span><CircleDot size={13} /> {map.events.length} 个事件</span><span><AppWindow size={13} /> {projectMounted ? '游戏目录已挂载' : '演示数据'}</span></div>
          </div>
        )}
